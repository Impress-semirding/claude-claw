...
- 📁 **文件隔离** - 每个会话有独立的工作目录
- 🌊 **流式响应** - 支持 SSE 流式输出
- 📱 **IM 集成** - 支持飞书、Telegram、QQ、钉钉、微信等（可选）
- 🌐 **Web 前端适配器** - 提供 HappyClaw Web 前端适配器，可直接对接使用

## Web 前端适配器

Claw 提供了专门的 Web 前端适配器，可以将 HappyClaw Web 前端与 Claw 后端无缝对接。
...
```bash
# 1. 复制适配器到 HappyClaw Web 项目
cp -r claw/web-adapter happyclaw/web/src/adapter

# 2. 配置环境变量
echo "VITE_CLAW_API_URL=http://localhost:3000" >> happyclaw/web/.env.local
