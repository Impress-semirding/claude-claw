...
export class ClaudeAgent {
  ...
  async buildSystemPrompt(env: AgentEnvironment): Promise<string> {
    ...
    const memoryRecallPrompt = [
      ...
      '根据信息的**时效性**选择存储位置：',
      '',
      `#### 全局记忆（永久）→ 直接编辑 \`${userGlobalPath || '/workspace/global/CLAUDE.md'}\``,
      '',
      ...
      `> - 是 / 可能 → **全局记忆**（编辑 \`${userGlobalPath || '/workspace/global/CLAUDE.md'}\`）`,
      '> - 明确只跟今天有关 → 日期记忆（`memory_append`）',
      '> - 用户说「记住这个」→ **一定写全局记忆**',
      '',
      '#### 强制工作流',
      `如果用户当前消息包含任何应写入全局记忆的信息（身份、职业、技术栈、偏好、明确要求「记住」的内容），你在输出任何文本回复之前，**必须**依次执行：`,
      `1. 使用 \`Read\` 读取 \`${userGlobalPath || '/workspace/global/CLAUDE.md'}\``,
