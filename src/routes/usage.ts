...
export default async function usageRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/stats', { preHandler: authMiddleware }, async (request, reply) => {
    try {
      ...
      const userId = query.userId || (request.user as { userId: string }).userId;
      ...
      return reply.status(500).send({ error: error instanceof Error ? error.message : 'Failed to load usage stats' });
    }
  });

  // GET /api/usage/models - 获取可用模型列表
  fastify.get('/models', { preHandler: authMiddleware }, async (request, reply) => {
    try {
      return reply.send({
        models: [
      ...
      return reply.status(500).send({ error: error instanceof Error ? error.message : 'Failed to load models' });
    }
  });
