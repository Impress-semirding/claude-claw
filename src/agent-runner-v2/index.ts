...
function writeOutput(output: ContainerOutput): void {
  if (outputMode === 'claw') {
    ...
    if (output.streamEvent) {
      console.log(JSON.stringify({ __claw_event__: true, ...output.streamEvent }));
      return;
    }
    if (output.status === 'success' && output.result !== null && output.result !== undefined && output.result !== '') {
      console.log(JSON.stringify({ type: 'assistant', content: output.result }));
      return;
    }
    if ((output.status === 'success' || output.status === 'stream') && output.newSessionId) {
      console.log(JSON.stringify({ type: 'result', session_id: output.newSessionId, last_assistant_uuid: output.sdkMessageUuid }));
      return;
    }
    // Other claw outputs that don't map (e.g. empty success) are silently dropped
    return;
  }
...
function log(message: string, level: 'trace' | 'debug' | 'info' | 'warn' | 'error' = 'info'): void {
  console.error(JSON.stringify({ level, msg: message, source: 'agent-runner', timestamp: new Date().toISOString() }));
}
...
function createPreCompactHook(
  ...
  return async (input, _toolUseId, _context) => {
    ...
    if ((preCompact as any).agent_id) {
      log(`PreCompact: skipping sub-agent compact (agent_id=${(preCompact as any).agent_id})`);
      return {};
    }
...
async function runQuery(
  ...
  try {
    ...
    if ((message as any).type === 'tool_use_summary') {
      if (!suppressOutputAfterInterrupt) {
        visibleOutputStarted = true;
      }
      if (suppressOutputAfterInterrupt) {
    ...
    if (message.type === 'system' && message.subtype === 'init') {
      ...
      if (outputMode === 'claw') {
        emit({ status: 'stream', result: null, streamEvent: { eventType: 'init', sessionId: newSessionId } });
      }
    }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (parsed.mcpEnv !== undefined || parsed.ipcDir !== undefined) {
      outputMode = 'claw';
      clawInput = parsed as ClawRunnerInput;
      containerInput = {
        prompt: clawInput.prompt,
        sessionId: clawInput.options?.resume || undefined,
        chatJid: clawInput.mcpEnv?.chatJid || 'web:main',
      ...
      containerInput = parsed as ContainerInput;
    }
  ...
  try {
    while (true) {
      ...
      if (queryResult.interruptedDuringQuery) {
        ...
        // 不 break，等待下一条消息
        if (outputMode === 'claw') {
          log('Claw mode: exiting after interrupt');
          writeOutput({ status: 'closed', result: null });
          break;
        }
      ...
      if (outputMode === 'claw') {
        log('Claw mode: exiting after query completion');
        writeOutput({ status: 'closed', result: null });
        break;
      }
