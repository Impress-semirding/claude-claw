  ...
  const tools: SdkMcpToolDefinition<any>[] = [
    ...
    tool(
      ...
      async (args) => {
        ...
        if (ctx.outputMode === 'claw') {
          console.error(JSON.stringify({ __mcp__: true, type: 'send_message', content: args.text, chatJid: ctx.chatJid }));
      ...
      async (args) => {
        ...
        if (ctx.outputMode === 'claw') {
          console.error(JSON.stringify({
            __mcp__: true,
            type: 'schedule_task',
            name: args.prompt?.trim() || 'Scheduled task',
            cron: `${args.schedule_type}:${args.schedule_value}`,
            prompt: args.prompt || '',
            chatJid: targetJid,
          }));
          return {
            content: [
              {
                type: 'text' as const,
                text: `Task scheduled [${modeLabel}]: ${args.schedule_type} - ${args.schedule_value}`,
              },
            ],
          };
        }
