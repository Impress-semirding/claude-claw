...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  handleRunnerState: (chatJid, state) => {
    if (state === 'idle') {
      ...
      // 不再立即 clearStreaming：保留流式内容（thinking + partialText）直到
      // new_message 完成最终转换，避免内容提前消失导致的抖动。
      // 先取消待处理的 rAF，防止 new_message 之后仍有陈旧 flush 重新创建 streaming。
      const mainKey = `main:${chatJid}`;
      ...
      set((s) => ({ waiting: { ...s.waiting, [chatJid]: false } }));
